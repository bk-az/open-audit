...
class AttributesModel extends BaseModel
{
    ...
    public function dictionary(): object
    {
        ...
        $dictionary->sentence = APP_DISPLAY_NAME . ' enables you to create and use your own values for certain stored fields. For example, if you have another status than those provided, just add it and use it. Simple.';

        $dictionary->about = '<p>The Attributes endpoint allows you to add customized values to different attributes in ' . APP_DISPLAY_NAME . ', at the moment this feature works on the Class, Environment, Status and Type attributes on Devices, the Type attribute for both Locations and Orgs as well as the Menu Category for Queries. If you view an item of one of the previous types (say view a Location) you will notice the Type attribute must be selected from a drop-down box. This is where those values are stored. Hence, if you would like to add a new Type to be chosen for a Location, add it using the Attributes feature.</p>';

        $dictionary->notes = '<p>If you add a device type, to display the associated icon you will have to manually copy the .svg formatted file to the directory:<br><em>Linux</em>:/usr/local/' . APP_WEB_ALIAS . '/public/device_images<br><em>Windows</em>: c:\\xampp\\htdocs\\' . APP_WEB_ALIAS . '\\device_images<br> <br>If you add a location type, to display the associated icon you will have to manually copy the 32x32px icon to the directory:<br><em>Linux</em>:/usr/local/' . APP_WEB_ALIAS . '/public/images/map_icons<br><em>Windows</em>: c:\\xampp\\htdocs\\' . APP_WEB_ALIAS . '\\images\\map_icons</p><p>When the <i>resource</i> is a <code>device</code>, valid <i>types</i> are: <code>class</code>, <code>environment</code>, <code>status</code> and <code>type</code>. If the <i>resource</i> is <code>locations</code> or <code>org</code> the only valid <i>type</i> is <code>type</code>. If the <i>resource</i> is a <code>query</code> the only valid <i>type</i> is <code>menu_category</code>.</p>';
