...
class RolesModel extends BaseModel
{
     ...
     * builder to target the 'roles' table.
     */
     ...
     *
     * @param  int|null $id    The ID of the role to delete
    ...
    /**
     * Return supplementary data for a single role's read view (stub)
     *
    ...
    /**
     * Return supplementary data for the role create/edit form (stub)
     *
     * Reserved for future implementation. Currently returns an empty array;
     * no data is fetched from the database.
     *
    ...
    /**
     * Truncate the roles table, removing all rows
     *
     * The $table parameter is accepted for interface compatibility but is
     * ignored; the method always resets the 'roles' table.
     *
     ...
     *
     * @param  int|null          $id   The ID of the role to update
    ...
    /**
     * Build and return the data dictionary for the roles collection
     *
     * Constructs a stdClass describing the `roles` table for use by the
     * framework's help, validation, and API-documentation systems.
     * The returned object includes:
     *  - table       : the collection name ('roles')
    ...
    public function dictionary(): object
    {
        ...
        $dictionary->sentence = APP_DISPLAY_NAME . ' Enterprise includes User Roles, full role-based access control (RBAC) allowing fine-grained control over each user\'s experience.';

        $dictionary->about = '<p>Roles in ' . APP_DISPLAY_NAME . ' are a key item. A user has a primary Org as well as a list of Orgs they can access. A user combines this with a list of assigned Roles that define what actions they can take on items assigned to the Orgs they have access to. The combination of a users "orgs" and "roles" define what they can and cannot do within ' . APP_DISPLAY_NAME . '.<br> <br>
        Get the complete ' . APP_DISPLAY_NAME . ' Enterprise experience. Creating roles allows fine-grained control over what your users can do within ' . APP_DISPLAY_NAME . '.<br> <br>
        The primary method for authorisation (what a user can do) is based on the users Roles. Default roles are defined as admin, org_admin, reporter and user. Each role has a set of permissions (Create, Read, Update, Delete) for each endpoint. The ability to define additional roles and edit existing roles is enabled in ' . APP_DISPLAY_NAME . ' Enterprise.<br> <br>Roles can also be used with LDAP (Active Directory and OpenLDAP) authorisation. Enterprise licensed installations have the ability to customise the LDAP group for each defined role.<br> <br></p>';
