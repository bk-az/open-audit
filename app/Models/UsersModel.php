...
class UsersModel extends BaseModel
{
     ...
     * builder to target the 'users' table.
     */
    ...
    /**
     * Return supplementary data for a single user's read view
     *
     * @param  int   $id The ID of the user whose supplementary data to load
     *
    ...
    /**
     * Return supplementary data for the user create/edit form
     *
     * @param  int   $id The ID of the user whose supplementary data to load
     *
     ...
     *
     * @return array        An array of formatted users entries
     */
    ...
    /**
     * Read every user from the database with no org-based filtering
     *
     * Returns all rows from the `users` table with no additional filtering.
     * Use {@see listUser()} when results should be restricted to the current user's accessible orgs.
     *
     * @return array  Array of stdClass objects representing every user row
     */
    ...
    /**
     * Truncate the users table, removing all rows
     *
     * The $table parameter is accepted for interface compatibility but is
     * ignored; the method always resets the 'users' table.
     *
    ...
    /**
     * Validate the currently logged-in session user and return their user object
     *
     * Checks whether a user is logged in via the session. If not, redirects to
     * the logon page and returns. If logged in, retrieves the user record, decodes
     * their roles and orgs JSON, maps roles to permissions, and returns the populated
     * user object. Returns false if the user record cannot be found.
     *
     * @return object|false  The user object with decoded roles and orgs, or false on failure
     */
    ...
    /**
     * Build and return the data dictionary for the users collection
     *
     * Constructs a stdClass describing the `users` table for use by the
     * framework's help, validation, and API-documentation systems.
     * The returned object includes:
     *  - table       : the collection name ('users')
    ...
    public function dictionary(): object
    {
        ...
        $dictionary->sentence = APP_DISPLAY_NAME . " Enterprise includes Users &amp; Roles, full role-based access control (RBAC) allowing fine-grained control over each user's experience.";
        ...
        $dictionary->notes = 'If the following conditions are met:<br><ul><li>a Role has an assigned ad_group</li><li>an Org has an assigned ad_group</li><li>an LDAP Server has use_roles set to y</li><li>a user exists in LDAP (be it Active Directory or OpenLDAP) and is in the assigned ad_groups</li></ul>That user can log on to ' . APP_DISPLAY_NAME . ' without an account in ' . APP_DISPLAY_NAME . ' needing to be created. ' . APP_DISPLAY_NAME . ' will query the LDAP in question and if the user is in the required groups but not in ' . APP_DISPLAY_NAME . ', their user attributes (name, full name, email,  roles, orgs, etc) within ' . APP_DISPLAY_NAME . ' will be automatically populated, and they will be logged on.<br> <br>';
