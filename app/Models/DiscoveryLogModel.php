...
class DiscoveryLogModel extends BaseModel
{
     ...
     * builder to target the 'discovery_log' table.
     */
    ...
    public function getByDevice(int $id): array
    {
        ...
        $this->builder->select("TIME_TO_SEC(TIMEDIFF(MAX(discovery_log.timestamp), MIN(discovery_log.timestamp))) AS `command_time_to_execute`, discovery_log.ip AS `ip`, devices.id AS `id`, devices.type AS `type`, devices.name AS `name`, devices.domain AS `domain`, devices.os_family AS `os_family`, devices.serial AS `serial`, devices.status AS `status`, devices.last_seen_by AS `last_seen_by`, devices.last_seen AS `last_seen`, devices.manufacturer AS `manufacturer`, devices.class AS `class`, devices.os_group AS `os_group`, devices.icon AS `icon`, devices.identification AS `identification`, '' AS `message`", false);
        $this->builder->join('devices', 'discovery_log.device_id = devices.id', 'left');
        $this->builder->where('discovery_log.discovery_id', intval($id));
        $this->builder->where('discovery_log.ip != ', '127.0.0.1');
        $this->builder->where('discovery_log.device_id IS NOT NULL');
     ...
     *
     * @param  int|null $id    The ID of the discovery_log to delete
    ...
    /**
     * Return supplementary data for a single discovery_log's read view (stub)
     *
    ...
    /**
     * Return supplementary data for the discovery_log create/edit form (stub)
     *
     * Reserved for future implementation. Currently returns an empty array;
     * no data is fetched from the database.
     *
     ...
     *
     * @return array        An array of formatted discovery_log entries
     */
    ...
    /**
     * Truncate the discovery_log table, removing all rows
     *
     * The $table parameter is accepted for interface compatibility but is
     * ignored; the method always resets the 'discovery_log' table.
     *
     ...
     *
     * @param  int|null          $id   The ID of the discovery_log to update
    ...
    /**
     * Build and return the data dictionary for the discovery_log collection
     *
     * Constructs a stdClass describing the `discovery_log` table for use by the
     * framework's help, validation, and API-documentation systems.
     * The returned object includes:
     *  - table       : the collection name ('discovery_log')
