        ...
        if (!empty($id)) {
            if ($this->resp->meta->format !== 'html') {
                ...
                if ($this->resp->meta->collection !== 'components') {
                    ...
                    if ($this->resp->meta->collection === 'discoveries' and !empty($this->resp->meta->received_data->execute)) {
        ...
        if ($this->resp->meta->format !== 'html') {
            ...
            if ($this->resp->meta->collection === 'devices') {
                ...
                if ($this->resp->meta->format === 'json_data') {
                    ...
                    $components_array = array('arp', 'bios', 'certificate', 'cli_config', 'disk', 'dns', 'executable', 'file', 'ip', 'license', 'log', 'memory', 'module', 'monitor', 'motherboard', 'netstat', 'network', 'nmap', 'optical', 'pagefile', 'partition', 'policy', 'print_queue', 'processor', 'radio', 'route', 'san', 'scsi', 'server', 'server_item', 'service', 'share', 'software', 'software_key', 'sound', 'task', 'usb', 'user', 'user_group', 'variable', 'video', 'vm', 'warranty', 'windows');
