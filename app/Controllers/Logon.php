    ...
    public function createForm()
    {
        ...
        if (empty($server_os)) {
            ...
            if (php_uname('s') === 'Windows NT') {
                ...
                $command = 'nohup php ' . FCPATH . 'index.php news execute install > /dev/null 2>&1 &';
                @exec($command, $output);
            }
        }
        ...
        if (!empty($server_os) and $server_os === 'Windows NT' and !stripos($server_platform, 'server')) {
            ...
            log_message('info', 'Discovery queue limit set to 1 as running on Windows but not Server, running on: ' . $server_platform);
        }
        ...
        //         if (php_uname('s') === 'Windows NT') {
        //             $command = "%comspec% /c start /b c:\\xampp\\php\\php.exe " . FCPATH . "index.php news execute vulnerabilities";
        //             pclose(popen($command, 'r'));
        //         } elseif (php_uname('s') === 'Darwin') {
