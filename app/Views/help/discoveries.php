...
$intro = '<p>' . __('Discoveries are entries that enable you to run a discovery upon a network in a single click, without entering the details of that network each and every time.') . '</p>
<br>
...
<br>' . __('An Active Directory discovery will query Active Directory for a list of networks and scan the associated IPs just like a regular Subnet discovery.') . '<br>

<br>' . __('A Seed discovery uses an initial IP to retrieve the IPs of any other devices it knows about, adds those IPs to the list and scans those - again asking for any IPs those additional devices knows about. Rinse and repeat. It is intelligent enough to know if it has already scanned a given IP in its execution and discards any already scanned (or even on the list to be scanned).') . '</p>';

$body = '<br>
<h2>' . __('Assigning Discovered Devices') . '</h2>

<p>' . __('If you set the values for') . ' <code>devices_assigned_to_org</code> ' . __('and / or') . ' <code>devices_assigned_to_location</code> ' . __('you will assign any found devices to that Org or Location.') . '</p>
...
<p>' . __('Last up is WMI. For Windows as in Linux, a user with Administrator level access is preferred (see Target Client Configuration on the wiki).') . '</p>
...
<p>' . __('We also have an option to Ping Before Scan (important for routers with long lived ARP caches). This is usually a good idea.') . '</p>

<p>' . __('All regular discovery options are also available for use in Device Seed Discoveries.') . '</p>
...
<p>' . __('How does a Device Seed Discovery find known IPs?') . '

<br>' . __('Provided you have the correct credentials, Device Seed Discovery works as follows') . ':<br>

<br>' . __('For') . ' SNMP ' . __('devices') . ':<br>
<code>ipNetToMediaPhysAddress (1.3.6.1.2.1.4.22.1.2)<br>
ipNetToPhysicalPhysAddress (1.3.6.1.2.1.4.35.1.4.3.1.4)<br>
atPhysAddress (1.3.6.1.2.1.3.1.1.2)<br>
ipRouteEntry (1.3.6.1.2.1.4.21.1.1)<br>
</code>

<br>' . __('For') . ' SSH ' . __('devices') . ':<br>
<code>arp -an</code> ' . __('and') . ' <code>netstat -rn</code> commands.<br>

<br>' . __('For') . ' Windows ' . __('devices') . ':<br>
<code>arp -a</code> command.</pre><br>
...
<p>' . __('For further details information, see the wiki.') . '</p>
<br>
';
